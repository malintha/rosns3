...
static Gnuplot TestDeterministic (Ptr<PropagationLossModel> model)
{
...

  {
    ...
    for (double distance = 0.0; distance < 250.0; distance += 1.0)
      {
...
static Gnuplot TestProbabilistic (Ptr<PropagationLossModel> model, unsigned int samples = 1000)
{
  Ptr<ConstantPositionMobilityModel> a = CreateObject<ConstantPositionMobilityModel> ();
  Ptr<ConstantPositionMobilityModel> b = CreateObject<ConstantPositionMobilityModel> ();
...
//   dataset.SetErrorBars();
  dataset.SetStyle ("with yerrorbars");
...
int main (int argc, char *argv[])
{
...

  {
    ...
    // log->SetAttribute ("Exponent", DoubleValue (2.52));
    // log->SetAttribute("ReferenceLoss", DoubleValue(-53));
...

  {
    ...
    // log_combined->SetAttribute ("Exponent", DoubleValue (2.52));
    // log_combined->SetAttribute("ReferenceLoss", DoubleValue(-53));

    // lossmodel->SetNext(log);
